...
Installation:
pip install streamlit pandas numpy matplotlib seaborn scikit-learn anthropic plotly scipy
...
class DataAnalystAgent:
    ...
    def __init__(self, api_key=None):
        ...
        # Initialize client only if valid API key is provided
    ...
    def get_ai_insights(self, context, question=None):
        ...
        if not self.api_key or not self.client:
            return "⚠️ Please provide a valid Anthropic API key to use AI insights."
...
Be specific, technical where appropriate, and provide clear recommendations."""
        ...
        try:
            ...
            return f"❌ **Authentication Error**: Invalid API key. Please check your API key and try again.\n\nDetails: {str(e)}"
...
def main():
    ...
    with st.sidebar:
        ...
        try:
            if "ANTHROPIC_API_KEY" in st.secrets:
                ...
                if secret_key and secret_key.strip().startswith("sk-ant-"):
            ...
            # No secrets file (running locally)
            pass
        except Exception as e:
            pass
        
        # If no API key from secrets, ask user to input
        if not api_key:
            st.info("💡 Enter your Anthropic API key below")
            ...
            if user_key and user_key.strip():
                # Validate format
                if user_key.strip().startswith("sk-ant-"):
    ...
    if st.session_state.df is not None:
        ...
        # Create agent with proper API key
        ...
        with tab1:
            ...
            with col2:
                ...
                if type_analysis['high_cardinality']:
        ...
        with tab7:
            ...
            if not st.session_state.get('api_key'):
                ...
                # Show example of what the assistant can do
                st.markdown("### What the AI Assistant Can Do:")
                st.markdown("""
                - 📊 Explain patterns in your data
                - 🎯 Identify important features
                - ⚠️ Highlight data quality issues
                - 📈 Suggest next analysis steps
                - 💡 Provide actionable insights
                """)
            else:
                # Verify the agent has a valid client
                if not agent.client:
                    st.error("❌ API client not initialized properly. Please check your API key.")
                    ...
                    with col1:
                        if st.button("📊 What are the main patterns in this data?"):
                            st.session_state.chat_history.append({
                                'role': 'user',
                                'content': "What are the main patterns in this data?"
                            })
                            st.rerun()
                        if st.button("🎯 Which features are most important?"):
                            st.session_state.chat_history.append({
                                'role': 'user',
                                'content': "Which features are most important?"
                            })
                            st.rerun()
                    ...
                    with col2:
                        if st.button("⚠️ What data quality issues should I address?"):
                            st.session_state.chat_history.append({
                                'role': 'user',
                                'content': "What data quality issues should I address?"
                            })
                            st.rerun()
                        if st.button("📈 What visualizations should I create next?"):
                            st.session_state.chat_history.append({
                                'role': 'user',
                                'content': "What visualizations should I create next?"
                            })
                            st.rerun()
